...
if (!class_exists('EG_Delicious_TagRolls_Widget')) {
	...
	class EG_Delicious_TagRolls_Widget extends EG_Widget_200 {
	
		function EG_Delicious_TagRolls_Widget() {
			$widget_ops = array('classname' => 'widget_delicious_tagrolls', 'description' => 'RSS feeds to display Delicious Tags' );
			$this->WP_Widget('egdel_tagrolls', 'EG-Delicious TagRolls', $widget_ops);

			$fields = array(
					'title'		=> array( 'type'  => 'text',    'label' => 'Title'),
					'username'	=> array( 'type'  => 'text',	'label' => 'Delicious username'),
					'count'		=> array( 'type'  => 'numeric', 'label' => 'Quantity'),
					'font' 		=> array( 'type'  => 'comment', 'label' => 'Font parameters'),
					'min_size'	=> array( 'type'  => 'numeric', 'label' => 'Smallest size'),
					'max_size'	=> array( 'type'  => 'numeric', 'label' => 'Largest size'),
					...
					'flow' 		=> array('type' => 'radio',		'label' => 'Flow',
						'list'	  	=> array ( 'flat' => 'Flat', 'list' => 'List (UL/LI)')),
					'showitems' => array( 'type' => 'comment',  'label' => 'Show'),
					'name' 		=> array( 'type' => 'checkbox', 'label'	=> 'Show Delicious name'),
					'showadd'	=> array( 'type' => 'checkbox', 'label' => 'Show "Add me to your network"'),
					'showcount'	=> array( 'type' => 'checkbox', 'label' => 'Show Tags count')
			);

			$plugin_options = get_option(EG_DELICIOUS_OPTIONS_ENTRY);
			if ($plugin_options !== FALSE && isset($plugin_options['username'])) 
				$username = $plugin_options['username'];
			else
				$username = '';
		...
		function widget($args, $instance) {
			...
			$output = '';
			if ($this->is_visible($values) && $values['username'] != '') {
				...
				if ( is_wp_error($rss) ) {
					$output .= '<p>' . sprintf( __('<strong>RSS Error</strong>: %s'), $rss->get_error_message() ) . '</p>';
				}
				else {
					if ( !$rss->get_item_quantity() ) {
						$output .= '<p>' . __( 'An error has occurred; the feed is probably down. Try again later.' ). '</p>';
					}
					else {
						$tags_list = array();
						foreach ( $rss->get_items(0, $items) as $item ) {
							$tags_list[$item->get_title()] = $item->get_description();
						}
	...
	} // End of EG_Delicious_TagRolls_Widget
	
} // End of class_exists EG_Delicious_TagRolls_Widget



if (!class_exists('EG_Delicious_Badge_Widget')) {

	class EG_Delicious_Badge_Widget extends EG_Widget_200 {
	
		function EG_Delicious_Badge_Widget() {
			$widget_ops = array('classname' => 'widget_links', 'description' => 'Display Delicious Network badge' );
			$this->WP_Widget('egdel_badge', 'EG-Delicious Network badge', $widget_ops);

			$fields = array(
					'title'     => array( 'type' => 'text',      'label' => 'Title'),
					'username'	=> array( 'type' => 'text',	     'label' => 'Delicious username'),
					'icon'		=> array( 'type' => 'select',    'label' => 'Icon',
						'list'  => array('icon-none' => 'None',  'icon-s' => 'Small', 'icon-m' => 'Medium')),
					'showadd'	=> array( 'type' => 'checkbox',  'label' => 'Show "Add me to your network"'),
					'name' 		=> array( 'type' => 'checkbox',  'label' => 'Show Delicious name'),
					'itemcount' => array( 'type' => 'checkbox',	 'label' => 'Show bookmark count'),
					'nwcount'	=> array( 'type' => 'checkbox',	 'label' => 'Show network count'),
					'fancount'  => array( 'type'  => 'checkbox', 'label' => 'Show fan count')
				);
				...
				if ( is_wp_error($rss) ) {
					...
					if ( !$rss->get_item_quantity() ) {
						...
						foreach ( $rss->get_items(0, $items) as $item ) {
							...
							$field_name  = $item->get_id();
							$field_value = $item->get_description();
	...
	} // End of EG_Delicious_Badge_Widget
	
} // End of class_exists EG_Delicious_Badge_Widget
...
function eg_delicious_widgets_init() {
	...
	register_widget('EG_Delicious_Badge_Widget');
	register_widget('EG_Delicious_TagRolls_Widget');
}
